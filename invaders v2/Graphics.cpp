	...
	strPool(d3D.GetDevice())
{
	...
	if (d3D.ComputeSupport())
	{
		copyPass = make_unique<CopyPass>(RM::Get().GetShader<CopyComputeShader>(), width, height);
		blurPass = make_unique<BlurPass>(device, width, height);
		bloomPass = make_unique<BloomPass>(device, width, height);
		target = make_unique<RenderTarget>(device, width, height);
	}
